...
try {
    ...
    foreach ($pembayaran_list as $index => $pembayaran) {
        ...
        echo '<button onclick="hapusPembayaran(\'' . addslashes($pembayaran['nikk']) . '\', \'' . addslashes($pembayaran['kode_tarif']) . '\', \'' . addslashes($pembayaran['bulan'] ?: 'Tahunan') . '\', \'' . addslashes($pembayaran['tahun']) . '\', \'' . addslashes($pembayaran['jml_bayar']) . '\', \'' . addslashes($pembayaran['tgl_bayar']) . '\')" class="text-red-600 hover:text-red-800 font-bold py-1 px-1">';
