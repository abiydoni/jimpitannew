...
if ($hasSendParam || $isCronJob) {
    ...
    if ($isCronJob) {
        ...
        if ($httpCode === 200) {
            error_log('auto_send_jaga.php: Pesan berhasil dikirim ke grup Telegram');
        } else {
            error_log('auto_send_jaga.php: Gagal mengirim pesan. HTTP Code: ' . $httpCode . ', Error: ' . ($curlError ?: 'Unknown'));
        }
    } else {
