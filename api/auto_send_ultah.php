...
if (empty(trim($message)) || stripos($message, 'kosong') !== false) {
    error_log('auto_send_ultah.php: Pesan kosong atau berisi "kosong", tidak ada yang dikirim');
    ob_end_clean();
    if ($hasSendParam || $isCronJob) {
...
if ($httpCode === 400 && $response) {
    ...
    if (isset($responseData['description']) && 
        ...
        error_log('auto_send_ultah.php: Markdown error, mencoba tanpa parse_mode');
        $payloadNoMarkdown = [
            'chat_id' => $chatId,
            'text'    => $message,
        ];
...
if ($hasSendParam || $isCronJob) {
    ...
    if ($isCronJob) {
        ...
        if ($httpCode === 200) {
            error_log('auto_send_ultah.php: Pesan berhasil dikirim ke grup Telegram (Chat ID: ' . $chatId . ')');
        } else {
            ...
            error_log('auto_send_ultah.php: Gagal mengirim pesan. HTTP Code: ' . $httpCode . ', Error: ' . $errorMsg . ', Chat ID: ' . $chatId);
            if ($responseData) {
                error_log('auto_send_ultah.php: Response: ' . json_encode($responseData));
            }
        }
    } else {
